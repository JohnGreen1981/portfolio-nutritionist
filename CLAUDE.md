...
Очищенный портфельный репозиторий Telegram-бота нутри-помощника.
...
Проект представлен как AI-assisted product automation prototype. В портфолио акцент на product workflow, stateful onboarding, контекст для LLM, tool-calling, privacy-aware data model, Telegram UX и scheduled digest.

Не описывать проект как медицинский продукт или замену врача/диетолога.

## Privacy / Health Safety

В публичный репозиторий не входят реальные `.env`, `keys.md`, Supabase credentials, пользовательские профили, история питания, фотографии еды, health data, chat logs и production deployment details.
